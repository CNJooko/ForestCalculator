...
## v1.7.6 (2026-06-09) — 虚拟滚动实现

### 性能优化
- `ui.js` `renderBatch()`: 重构渲染逻辑，支持虚拟滚动
  - 新增 `_renderIncrementalBatch()`: 保留原有增量更新逻辑（行数 ≤ 50 时使用）
  - 新增 `_renderVirtualBatch()`: 虚拟滚动核心（行数 > 50 时启用）
  - 新增 `_bindVirtualScroll()`: 绑定滚动事件 + 防抖处理
  - 阈值判定: 行数 > 50 自动启用虚拟滚动
  - 绝对定位 + top 值定位行，只渲染可视区域 + 缓冲区（上下各 5 行）
  - 滚动事件 16ms 防抖（~60fps）
- `style.css` `.table-wrap`: 添加 `position: relative` 为虚拟滚动提供定位基准

### 技术细节
- **固定行高**: 52px（与 CSS `tbody td { height: 52px }` 保持一致）
- **缓冲区域**: 上下各 5 行，确保滚动流畅
- **DOM 复用**: 通过 `_batchRowPool` 缓存池复用已创建的行
- **变更检测**: 通过 `dataset` 属性缓存行状态
- **自动回退**: 行数 ≤ 50 时自动使用原有增量更新模式

### 性能提升（理论估算 vs v1.7.4）
- 100 行：无变化（使用增量更新）
- 500 行：首次渲染提升 ~60-80%（只渲染 ~30 行 DOM）
- 1000 行：首次渲染提升 ~80-90%（只渲染 ~30 行 DOM vs 1000 行）
- 增量更新（编辑单行）：提升 ~70-80%

## v1.7.5 (2026-06-09) — 性能监控集成

### 新增功能
- `ui.js`: 添加性能监控功能（`_logPerf()`）
  - 在 `renderBatch()` 和 `calcBatch()` 入口记录开始时间
  - 在函数末尾计算耗时并调用 `_logPerf()` 记录
  - 性能数据存储在 `ForestCalc._perfLog` 数组（最多保留 50 条）
  - 控制台输出格式：`[Perf] renderBatch: 12.34 ms`

### 技术细节
- 使用 `performance.now()` 进行高精度计时（微秒级）
- 性能日志包含：函数标签、耗时（ms）、时间戳
- 非侵入式设计：不影响现有功能和用户体验
